...
def consulta1(connect):
    ...
    from 
	(select extract(month from cons.data_volta) as mes, count(reg_vac.cod_vacina) as total_vacina
	from ANIMAL as ani, CONSULTA as cons, REGISTRO_VACINA as reg_vac
	where ani.cod_animal = cons.cod_animal and 
	      cons.id_consulta = reg_vac.id_consulta and 
	 	  ani.porte = 'Pequeno' and
		  cons.data_volta between '2022-04-01' and '2022-06-30'
	group by mes) as vacina,
	(select extract(month from cons.data_volta) as mes, sum(cons.valor) as despesa_veterinario
...
def consulta2(connect):
    select_query = """
    select extract(month from ani_res.data_resgate) as n_mes, 
       to_char(ani_res.data_resgate, 'TMMonth') as mes_resgate_animal, 
